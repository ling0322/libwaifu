...
CATCH_TEST_CASE("test Metal device transfer round trip", "[op][metal]") {
  ...
  Tensor a = cpuOps()->rand({3, 7, 5}, DType::kFloat);
  CATCH_REQUIRE(cpuOps()->allClose(
      metalOps()->toDevice(Device::getCpu(), metalOps()->toDevice(Device::getMetal(), a)),
      a));
}
...
CATCH_TEST_CASE("test Metal device transfer (fp32 -> fp16 -> fp32)", "[op][metal]") {
  ...
  Tensor a = cpuOps()->rand({3, 7, 5}, DType::kFloat);
  Tensor metalHalf = metalOps()->cast(metalOps()->toDevice(Device::getMetal(), a), DType::kFloat16);
  Tensor back = metalOps()->toDevice(Device::getCpu(), metalOps()->cast(metalHalf, DType::kFloat));
...
CATCH_TEST_CASE("test Metal device transfer (contiguous after transpose)", "[op][metal]") {
  ...
  Tensor a = cpuOps()->rand({4, 8, 16}, DType::kFloat);
  Tensor at = cpuOps()->contiguous(a.transpose(2, 1));
  Tensor metalT = metalOps()->toDevice(Device::getMetal(), at);
  Tensor back = metalOps()->toDevice(Device::getCpu(), metalT);
