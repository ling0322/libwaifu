...
`CLIPTokenizerFast`, reading the same repository's `tokenizer.json`, does not -- no `tokenizer.json`
can say "run ftfy" -- so the two upstream tokenizers give different ids for the same text. This is
huggingface's difference rather than one of ours, and it is not going to be reconciled here.

A package carries `tokenizer.json`, so the runtime is the fast one exactly. That is the right half
to be: diffusers loads the fast tokenizer, so it is the one these weights are actually driven by.

What remains open is only that the slow one exists and differs. Measured over 1600 texts
(2026-08-27, against the encoder that has since been deleted): 1271 ASCII texts agreed, and the
137 that disagreed were exactly the 137 ftfy rewrites -- `ﬁ` to `fi`, `ｆｕｌｌ` to `full`, `ǅ`
into NFC. The NFC half of that list is now handled, because the normalizer is in the file; the
mojibake-repair half is what ftfy alone does.

Worth knowing: `CLIPTokenizer` falls back to a `BasicTokenizer` when ftfy is absent, and that one
puts spaces between CJK characters -- so the slow tokenizer's output for `霧雨魔理沙` depends on
what is pip-installed beside it. The fast one has no such dependency, which is a second reason to
be it.

## ~~Anima's two tokenizers do not normalize text the way theirs do~~ (fixed 2026-09-13)

Qwen3 applied NFC before matching its pattern and `PreTokenizer::Qwen2` did not; T5's sentencepiece
model names `nmt_nfkc`, and `unigram.rs` implemented the whitespace handling and none of the
folding. `ｆｕｌｌwidth` came out here as `▁`, `<unk>`, `wid`, `th` where sentencepiece gave
`▁full`, `wid`, `th`.

Both are gone with the encoders that had them. The package carries each model's published
`tokenizer.json`, the normalizer is one of the things written in it, and the `tokenizers` crate
applies it -- including the `precompiled_charsmap` that is T5's `nmt_nfkc`, which was the half that
was a generated table rather than a rule. Verified by encoding the exported files from both sides:
614 texts each through Qwen3 and T5, and 1250 through CLIP, agreeing with `transformers` on every
one, fullwidth and ligature cases included.
...
```bash
cmake -S . -B build -DWITH_CUDA=ON -DCMAKE_CUDA_ARCHITECTURES=120a-real
```
...
## ~~`WITH_CUDA=ON` on its own does not link, and the README says it needs nothing~~ (fixed 2026-09-13)
...
`#ifdef LIBWAIFU_CUTLASS_ENABLED` around it, or `WITH_CUDA` should imply `WITH_CUTLASS`.
...
The README was rewritten in the same pass: the CUDA section names `install_cutlass.sh`, the
Conv2d section says CUTLASS is the only implementation and cuDNN is the benchmark's reference,
and `LIBWAIFU_CONV`, which no code has ever read, is gone from it.


## ~~`pagedAttention` aborts the process where `attention` beside it falls back~~ (fixed 2026-09-11)

Without FlashAttention, `CudaOperators::pagedAttention` ended in a bare `NOT_IMPL()`, which is
`LOG(FATAL)` then `abort()`, so `stores_and_reads_a_paged_kv_cache` killed the `tensor_cuda`
runner and took the ten tests after it down with it. There is still no portable paged attention
to fall back to -- `Operators::pagedAttention` is itself `NOT_IMPL()` -- but it now throws
