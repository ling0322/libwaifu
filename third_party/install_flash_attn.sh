...
# Said on the command line rather than left to a default, because a default can be pre-empted.
# CMake reads CMAKE_BUILD_TYPE from the environment since 3.22, and something on a GitHub
# Windows runner sets it to Debug -- which is how this archive came to be compiled against the
# debug CRT, and how it then refused to link beside a release build with an LNK2038 that named
# a file in this project rather than the environment that chose for it. An argument here beats
# both the environment and any default the CMakeLists might offer.
cmake_args=(
    -S "${FLASH_ATTN_DIR}"
    -B "${BUILD_DIR}"
    -DCMAKE_BUILD_TYPE="${FLASH_ATTN_BUILD_TYPE:-Release}")
